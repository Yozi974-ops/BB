...
import {
  ...
  saveProperty as savePropertyService, uploadPropertyDocument
...
export const AddPropertyProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const saveProperty = useCallback(async (): Promise<void> => {
    const draft = propertyRef.current;

    // 1) Création du bien côté backend
    const res = await savePropertyService(draft);

    // 2) Upload des documents si tu as des URIs locales
    if (draft.documents && draft.documents.length > 0) {
      for (const doc of draft.documents) {
        // adapte si ta structure document est différente
        if ((doc as any).uri) {
          try {
            await uploadPropertyDocument(res.id, (doc as any).uri, doc.name || "document.pdf");
          } catch (e) {
            console.log("Erreur upload doc", doc.name, e);
          }
        }
      }
    }

    // 3) Nettoyer le brouillon après succès (optionnel mais logique)
    try {
      await clearDraftService();
    } catch (e) {
      console.log("Erreur clearDraft après save", e);
    }
